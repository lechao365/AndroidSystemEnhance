 ...
 */
void IoHalImpl::refresh_devices() {
 ...
 */
DeviceEntry* IoHalImpl::resolve_device(int minor) {
 ...
 * 便于上层监控遍历与测试断言。
 */
ndk::ScopedAStatus IoHalImpl::listDevices(std::vector<std::string>* _aidl_return) {
    refresh_devices();
    _aidl_return->clear();
 ...
 */
ndk::ScopedAStatus IoHalImpl::resetState(int32_t in_deviceMinor) {
    auto* entry = resolve_device(in_deviceMinor);
 ...
 */
ndk::ScopedAStatus IoHalImpl::getConfig(int32_t in_deviceMinor, IoConfig* _aidl_return) {
    *_aidl_return = {};
    auto* entry = resolve_device(in_deviceMinor);
 ...
 * 返回 bool 表示内核是否接受配置。
 */
ndk::ScopedAStatus IoHalImpl::setConfig(int32_t in_deviceMinor, const IoConfig& in_config, bool* _aidl_return) {
    *_aidl_return = false;
    auto* entry = resolve_device(in_deviceMinor);
