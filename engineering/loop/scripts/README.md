...
| 脚本 | 作用 | 调用方式 |
|------|------|---------|
| [`le.sh`](./le.sh) | Loop Engineering CLI wrapper，底层调 `loop_core.cli` | `/le` 或 `bash engineering/loop/scripts/le.sh run` |
...
| 入口 | 作用 | 调用方式 |
|------|------|---------|
| `le.sh` | LE CLI 主入口（fixture/live） | `le.sh run --suite ... --fixture/--host ...` |
