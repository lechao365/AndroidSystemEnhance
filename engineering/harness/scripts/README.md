...
`le.sh` / `le_runs_cleanup.sh` / `rp5_serial_helper.py` / `start_rp5_serial_host.bat` 属于 loop engineering 闭环，已迁至 [`engineering/loop/scripts/`](../../loop/scripts/README.md)，本目录不再承载。Windows `.bat` 格式注意事项（CRLF / 纯 ASCII）同样移至该 README。
