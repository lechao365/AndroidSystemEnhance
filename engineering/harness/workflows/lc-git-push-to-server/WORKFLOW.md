...
- 当前位于 git 仓库工作目录
- 远程 `origin` 可达（`--no-push` 模式不要求）
- 改动符合 `SRC-001` 源码改动优先级（workspace 先行）与归档纪律
...
**单次确认门（强制，不可跳过）**——`commit_and_push.sh` 执行前：
...
- git commit（本地）+ push 到 `origin/<branch>`
- `commit_and_push.sh` 终端摘要 + `log_result` 结构化记录（commit hash、分支、remote）
- 日志/artifacts 按 `OBS-001`/`OBS-002` 落入 `engineering/output/log/commit_and_push/`
...
- `SRC-001`：workspace 源码改动优先级（提交前应已完成归档纪律）
- `OBS-001` / `OBS-002`：脚本维测（bootstrap 接入、统一退出码 0/1/2/4）
...
### 3. 单次确认（详见上方 Human confirmation gates）
