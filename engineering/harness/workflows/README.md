...
| [sync-code-to-patchs](./sync-code-to-patchs/) | workspace 源码归档到 patchs 镜像 | patchs 是 workspace 定制改动的精确镜像（含删除对齐） | `sync_code_to_patchs.sh` |
| [revert-code-from-patchs](./revert-code-from-patchs/) | workspace 坏了，从 patchs 基线回退 | sync 的逆操作，patchs 为已知良好基线（灾难恢复） | `revert_code_from_patchs.sh` |
...
- 日志：`engineering/harness/log/<script-name>/latest.log`
- 中间产物（manifest、plan、verify、build-report 等）：`engineering/harness/log/<script-name>/artifacts/<ts>-<name>`
