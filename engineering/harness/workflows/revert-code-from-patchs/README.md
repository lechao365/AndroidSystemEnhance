...
- **职责边界**：`sync-code-to-patchs` 的逆操作；`patchs/rpi5/` 是已知良好基线（promoted baseline），workspace 是操作对象。
...
| 文件 | 职责 | 关键入口 |
|------|------|---------|
| `WORKFLOW.md` | workflow 契约：trigger / preconditions / 受控例外说明 / 计划→确认→执行→校验 | 被 `.opencode/commands/revert-code-from-patchs.md` `@` 消费 |
...
| 触发方式 | 说明 |
|---------|------|
| `/revert-code-from-patchs` | 生成回退计划 → AI 逐条确认 → 执行 → 落盘校验 |
