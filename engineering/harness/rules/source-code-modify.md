...
> - `SRC-002`：`patchs/`（除 `others/`）是 workspace 的单向受控归档目录，仅允许通过 `lc-sync-code-to-patchs` 同步，严禁手动修改。
...
| 目标 | 操作流程 |
|------|---------|
| `patchs/` 下 `kernel/`、`aosp/` 等（对应 workspace 源码） | **必须先改 `~/workspace/` 源码**，验证通过后通过 `lc-sync-code-to-patchs` 命令（`/lc-sync-code-to-patchs`）同步归档 |
...
除 `others/` 外，**`patchs/` 下的目录只允许通过 `lc-sync-code-to-patchs` 命令（`/lc-sync-code-to-patchs`）同步，严禁手动修改**。原因：
...
| 禁止行为 | 原因 | 正确做法 |
|---------|------|---------|
| 未打包/未上板就执行 `lc-sync-code-to-patchs` | 归档未验证代码，违反"验证后归档"原则 | 走完编译→打包→上板→验证全流程 |
