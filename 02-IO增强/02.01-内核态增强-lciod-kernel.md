...
| `depends on` | `USB_STORAGE` | 依赖 usb-storage |
...
生命周期：`device_alloc` 初始化 kref=1 → `open()` kref_get → USB 断开 kref_put（但 fd 仍持有，引用>0）→ `close()` kref_put → 引用归零触发 `device_release`（ida_free + kfree）。
