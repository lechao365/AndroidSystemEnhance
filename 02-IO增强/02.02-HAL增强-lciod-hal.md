...
| 模块 | 职责 | 关键抽象 |
|------|------|---------|
| `hal_service.cpp` | AIDL 接口实现、设备缓存管理、字段映射 | `class IoHalImpl` — [hal_service.cpp:82](./../patchs/rpi5/aosp/new/vendor/lechao/services/lechao_lciod/hal/hal_service.cpp#L82) |
...
| `vendor_lechao_usbd-ioctl.h` | 内核-用户态共享 ABI | `struct vendor_lechao_usbd_stats` — [vendor_lechao_usbd-ioctl.h:45](./../patchs/rpi5/aosp/new/vendor/lechao/services/lechao_lciod/hal/vendor_lechao_usbd-ioctl.h#L45) |
...
| 文件 | 职责 | 关键函数/类 | 行数 |
|------|------|------------|------|
| `hal_service.cpp` | AIDL 实现、设备缓存、字段映射、进程入口 | `IoHalImpl` / `main()` | 352 |
| `device_io.cpp` | 底层 IO 封装（glob 枚举、带重试 open、ioctl、排空 read） | `list_devices()` / `read_event()` | 162 |
| `device_io.h` | IO 操作 C 风格 API 声明 | — | 85 |
| `vendor_lechao_usbd-ioctl.h` | 内核共享 ABI（struct/event/ioctl 命令） | `struct vendor_lechao_usbd_stats` | 166 |
...
| `lechao_lciod_hal` | `cc_binary` | HAL 进程二进制，`vendor: true`，链接 `vendor.lechao.lciod-V1-ndk` |
...
```cpp
// hal_service.cpp:286 — readEvent 中懒打开持久 fd
if (entry->fd < 0) {
    entry->fd = ::open_device(entry->path.c_str());
}

// hal_service.cpp:298 — 设备移除时关闭 fd 并标记 -1
...
```cpp
// device_io.cpp:130 — poll + 循环 read 排空
...
```cpp
// hal_service.cpp:180 — getStats 中的字段映射
...
| `open_device(path)` | `const char*` | `int fd` | 带重试打开（10次 × 200ms） |
...
| 常量 | 值 | 定义位置 | 说明 |
|------|-----|---------|------|
| `OPEN_RETRY_MAX` | 10 | `device_io.cpp:43` | open 重试次数 |
| `OPEN_RETRY_DELAY_MS` | 200ms | `device_io.cpp:44` | open 重试间隔 |
| `DEV_PREFIX` | `/dev/vendor_lechao_usbd` | `device_io.cpp:22` | 设备节点路径前缀 |
| `EVENT_BUF_SIZE` | 32 | `vendor_lechao_usbd-ioctl.h:103` | 内核事件环形缓冲区大小 |
| `IOC_MAGIC` | `'R'` | `vendor_lechao_usbd-ioctl.h:150` | ioctl magic number |
| Binder 线程池 | 1 | `hal_service.cpp:337` | 单线程串行执行 |
