...
| `raw-gadget.c` | Raw Gadget ioctl 封装 + 完整 USB 枚举（open/enumerate/stall/ep_read/ep_write/vbus） | `struct raw_gadget` / `raw_gadget_open()` / `raw_gadget_enumerate()` / `raw_gadget_stall_ep()` — [raw-gadget.c:21](./../patchs/rpi-zero2w/others/usb-fault-inject/raw-gadget.c) |
