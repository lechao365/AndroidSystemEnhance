...
**设计理念：** 完全接管 UDC 策略——通过 Raw Gadget 自行实现完整 USB 枚举（INIT → RUN → EVENT_FETCH → EP0 → SET_CONFIGURATION → EP_ENABLE），按入口层、协议处理层、描述符层、底层封装、故障实现、期望值、协议定义拆分为多文件架构，总计 2114 行。
...
| 模块 | 职责 | 关键抽象 |
|------|------|---------|
| `main.c` | CLI 入口 + 命令分发 + getopt_long 参数解析 | `main()` / `usage()` / `parse_corrupt_field()` / `parse_fault_name()` — [main.c:50](./../patchs/rpi-zero2w/others/usb-fault-inject/main.c) |
| `bot.c` | BOT 协议处理（CBW 接收 / Data 传输 / CSW 构造） | `bot_main_loop()` — [bot.c:27](./../patchs/rpi-zero2w/others/usb-fault-inject/bot.c) |
| `scsi.c` | SCSI 命令处理（INQUIRY / READ / WRITE 等） | `scsi_handle_command()` — [scsi.c](./../patchs/rpi-zero2w/others/usb-fault-inject/scsi.c) |
| `usb-descriptors.c` | USB 描述符操作 | `usb_desc_get_*()` — [usb-descriptors.c](./../patchs/rpi-zero2w/others/usb-fault-inject/usb-descriptors.c) |
| `raw-gadget.c` | Raw Gadget ioctl 封装 + 完整 USB 枚举（open/enumerate/stall/ep_read/ep_write/vbus） | `struct raw_gadget` / `raw_gadget_open()` / `raw_gadget_enumerate()` / `raw_gadget_stall_ep()` — [raw-gadget.c:15](./../patchs/rpi-zero2w/others/usb-fault-inject/raw-gadget.c) |
| `faults.c` | 11 类故障实现 + `fault_execute` 命令分发 | `fault_execute()` / `fault_id_to_name()` — [faults.c:31](./../patchs/rpi-zero2w/others/usb-fault-inject/faults.c) |
| `expect.c` | `expect_table[FAULT__MAX]` 期望值契约表 + JSON 输出 | `expect_table` / `expect_output_by_id()` / `expect_list_all()` — [expect.c:14](./../patchs/rpi-zero2w/others/usb-fault-inject/expect.c) |
...
| 不变量 | 保证机制 | 代码位置 |
|--------|---------|---------|
| CBW 签名验证 | `cbw.dCBWSignature != USB_MS_CBW_SIGNATURE` 时跳过处理 | [`bot.c:73-76`](./../patchs/rpi-zero2w/others/usb-fault-inject/bot.c) |
| CSW Residue 正确计算 | `csw.dCSWDataResidue = cbw.dCBWDataTransferLength - actually_transferred` | [`bot.c:194`](./../patchs/rpi-zero2w/others/usb-fault-inject/bot.c) |
| CSW Tag 回显 | `csw.dCSWTag = cbw.dCBWTag`（除非 F6 故意篡改） | [`bot.c:193`](./../patchs/rpi-zero2w/others/usb-fault-inject/bot.c) |
| Data 方向判断 | `sr.dir == SCSI_DIR_IN / SCSI_DIR_OUT` 由 SCSI 层决定 | [`bot.c:109/156`](./../patchs/rpi-zero2w/others/usb-fault-inject/bot.c) |
| Data 分块发送 | 每次 `raw_gadget_ep_write` 最多 512 字节 | [`bot.c:137-146`](./../patchs/rpi-zero2w/others/usb-fault-inject/bot.c) |
| ep_io 堆分配 | `usb_raw_ep_io` 使用 flexible array，必须 calloc 连续内存 | [`raw-gadget.c:92-94`](./../patchs/rpi-zero2w/others/usb-fault-inject/raw-gadget.c) |
