...
DAEMON -> DAEMON : extract_minor_from_path()
DAEMON --> UP : [0]
...
alt 每 200 tick（10s）
    ...
    MON -> MON : extract_minor_from_path()
end

MON -> HAL : readEvent(deviceMinor, 50ms)
alt 有效事件
...
| `IoHalClient` | HAL Binder 连接管理、延迟连接、指数退避重连、死亡通知 | `class IoHalClient` — [hal_client.h:22](./../patchs/rpi5/aosp/new/vendor/lechao/services/lechao_lciod/daemon/hal_client.h#L22) |
...
|------|---------------|---------------|------|
| `currentRate` | 包含（L42） | 省略 | 上层通过 `getAverageRate()` 接口按需计算 |
| `enabled` | 包含（L47） | 省略 | 管理字段，不暴露给上层 App |
| `flags` | 包含（L48） | 省略 | 管理字段，保留给内核扩展使用 |
...
repeat
    ...
    if (!hal?) then (是)
        ...
        if (tick % 200 == 0?) then (是)
            :listDevices();
            :extract_minor_from_path();
            :deviceMinor = minor;
        endif
        
        :readEvent(deviceMinor, 50ms timeout);
...
| 参数 | 值 | 定义位置 |
|------|-----|---------|
| tick 间隔 | 50ms | `service.cpp:211` |
| 统计刷新周期 | 200 tick（10s） | `service.cpp:217` |
| readEvent timeout | 50ms | `service.cpp:230` |
...
| 死亡回调清空缓存 | `onHalDied()` 中 `hal_.reset()` + `connected_ = false` | [`hal_client.cpp:65`](./../patchs/rpi5/aosp/new/vendor/lechao/services/lechao_lciod/daemon/hal_client.cpp#L65) |
| 指数退避上限 | `min(retryCount, 4)` 左移位，最大 5000ms | [`hal_client.cpp:77`](./../patchs/rpi5/aosp/new/vendor/lechao/services/lechao_lciod/daemon/hal_client.cpp#L77) |
| detach 线程随进程终止 | 进程为 oneshot，退出时线程自动终止 | [`service.cpp:202`](./../patchs/rpi5/aosp/new/vendor/lechao/services/lechao_lciod/daemon/service.cpp#L202) |
...
│   ├── hal_client.h             # IoHalClient 声明（mutex + 状态字段）
...
| 文件 | 职责 | 关键函数 | 行数 |
|------|------|---------|------|
| `service.cpp` | IoServiceImpl 实现、代理转发、字段投影、监控线程 | `IoServiceImpl::listDeviceMinors()` / `getAverageRate()` / `getIoStats()` / `start_monitor()` | 305 |
| `hal_client.cpp` | IoHalClient 实现、延迟连接、指数退避、死亡通知 | `IoHalClient::connect()` / `onHalDied()` / `get()` | 89 |
| `hal_client.h` | IoHalClient 声明 | `class IoHalClient` | 38 |
...
// service.cpp:115-143（字段投影）
// 省略 vstats.currentRate、vstats.enabled、vstats.flags
// 直传所有其他字段
```
...
// hal_client.cpp:76-81 — onUnlinked 回调释放 DeathRecipient + cookie
...
void start_monitor() {
    std::thread([this]() {
        ...
        while (true) {
            ...
            // 每 tick 轮询事件
            VendorIoEvent vev;
            hal->readEvent(deviceMinor, 50, &vev);
            // 每 200 tick 打印统计
            if (tick % 200 == 0) {
                hal->getStats(deviceMinor, &stats);
...
### 路径→minor 转换 — extract_minor_from_path

**设计思路：** HAL `listDevices()` 返回 `/dev/vendor_lechao_usbd<N>` 路径列表，system daemon 通过 `extract_minor_from_path()` 解析 minor 编号，上层只需感知整数编号而非底层设备路径。
...
:检查路径前缀;
if (匹配 "/dev/vendor_lechao_usbd"?) then (否)
    :返回 -1;
    stop
endif

:定位数字后缀位置;
:atoi(path + prefix_len);
...
static int extract_minor_from_path(const std::string& path) {
    ...
    return std::atoi(path.c_str() + sizeof(prefix) - 1);  // 解析数字后缀
}
```
