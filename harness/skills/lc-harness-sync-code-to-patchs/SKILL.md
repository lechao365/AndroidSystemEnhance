---
name: lc-harness-sync-code-to-patchs
...
# lc-harness-sync-code-to-patchs
...
- 用户请求"归档"/"同步 code"/"lc-harness-sync-code-to-patchs"
...
```bash
python3 harness/skills/lc-harness-sync-code-to-patchs/lc_harness_sync_code_to_patchs.py              # 全量镜像同步（默认含删除）
python3 harness/skills/lc-harness-sync-code-to-patchs/lc_harness_sync_code_to_patchs.py --check-only  # 仅检查，不执行（STALE 仅报告将删除项）
python3 harness/skills/lc-harness-sync-code-to-patchs/lc_harness_sync_code_to_patchs.py --no-prune    # 仅添加/更新，不删除对齐
```
