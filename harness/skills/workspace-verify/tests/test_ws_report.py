    ...
    def test_mode_a_normal(self):
        ...
        with redirect_stdout(buf):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 ...
                                 "--selfcheck", "pytest_rc=0 refs_rc=0 config_rc=0 contract_rc=0 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 | 120 passed, 2 skipped in 5.0s"])
    ...
    def test_selfcheck_failed_nonzero_rejected(self):
        ...
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 ...
                                 "--selfcheck", "pytest_rc=0 refs_rc=0 config_rc=0 contract_rc=0 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 | 1 failed, 119 passed, 2 skipped in 5.0s"])
        self.assertEqual(rc, 2)
    ...
    def test_selfcheck_missing_skipped_rejected(self):
        ...
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "skip", "--build", "skip",
                                 "--board", "skip", "--summary", "s",
                                 "--selfcheck", "pytest_rc=0 refs_rc=0 config_rc=0 contract_rc=0 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 | 120 passed in 5.0s"])
        self.assertEqual(rc, 2)
    ...
    def test_selfcheck_config_rc_nonzero_rejected(self):
        ...
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "skip", "--build", "skip",
                                 "--board", "skip", "--summary", "s",
                                 "--selfcheck", "pytest_rc=0 refs_rc=0 config_rc=1 contract_rc=0 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 | "
                                 "120 passed, 2 skipped in 5.0s"])
        self.assertEqual(rc, 2)
    ...
    def test_selfcheck_contract_rc_nonzero_rejected(self):
        ...
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "skip", "--build", "skip",
                                 "--board", "skip", "--summary", "s",
                                 "--selfcheck", "pytest_rc=0 refs_rc=0 config_rc=0 contract_rc=1 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 | "
                                 "120 passed, 2 skipped in 5.0s"])
        self.assertEqual(rc, 2)
        self.assertIn("非零退出码", err.getvalue())
    ...
    def test_selfcheck_pytest_rc_nonzero_rejected(self):
        ...
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "skip", "--build", "skip",
                                 "--board", "skip", "--summary", "s",
                                 "--selfcheck", "pytest_rc=1 refs_rc=0 config_rc=0 contract_rc=0 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 | "
    ...
    def test_selfcheck_refs_rc_nonzero_rejected(self):
        ...
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "skip", "--build", "skip",
                                 "--board", "skip", "--summary", "s",
                                 "--selfcheck", "pytest_rc=0 refs_rc=1 config_rc=0 contract_rc=0 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 | "
    ...
    def test_selfcheck_contradictory_refs_text_rejected(self):
        ...
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "skip", "--build", "skip",
                                 "--board", "skip", "--summary", "s",
                                 "--selfcheck", "pytest_rc=0 refs_rc=0 config_rc=0 contract_rc=0 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 | "
                                 "531 passed in 27.9s | skipped=0 | "
                                 "==== 共 3 处悬空引用（exit 1）===="])
        self.assertEqual(rc, 2)
    ...
    def test_selfcheck_four_rc_all_zero_passes(self):
        ...
        with redirect_stdout(buf):
            rc = ws_report.main(["--batch-file", batch, "--result", "skip",
                                 ...
                                 "ioctl_rc=0 | OK: 一致 | "
                                 "manifest_rc=0 | manifest.yaml 无变化"])
        self.assertEqual(rc, 0)
