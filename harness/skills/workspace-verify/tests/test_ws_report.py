...
def _selfcheck_ok():
    return ("pytest_rc=0 refs_rc=0 config_rc=0 contract_rc=0 pyenv_rc=0 "
            ...
            "ruff_rc=0 host_rc=0 metrics_rc=0 opencode_rc=0 quotepath_rc=0 "
            "known_issues_rc=0 commit_coverage_rc=0 lcview_events_rc=0 commit_coverage_rc=0 lcview_events_rc=0 | "
...
class TestWsReport(unittest.TestCase):
    ...
    def _write_acc(self, run_id="acc123", summary="svc:lechao_lcview boot",
                   start=1.0, end=2.0, overall="pass", items=None, **extra):
        # 自描述验收产物（方向 1/3）：run_id/输入摘要/设备信息/单调时间/逐项/总判定
        d = {
            "run_id": run_id,
            "input_summary": summary,
            "device_serial": "s1",
            "device_fingerprint": "f1",
            "start_monotonic": start,
            "end_monotonic": end,
            "items": items if items is not None else
                    [{"tag": "svc:lechao_lcview", "status": "pass",
                      "detail": "running"}],
            "overall": overall,
        }
        d.update(extra)
    ...
    def _write_ut(self, run_id="acc123", targets=None):
        # 自描述单测产物（方向 2）：run_id + 每 target 返回码/用例数/失败数
        d = {
            "run_id": run_id,
            "targets": targets if targets is not None else
                       [{"name": "t1", "rc": 0, "tests": 42, "failed": 0}],
        }
        return self._write(json.dumps(d, ensure_ascii=False), ".json")

    def _write_push(self, run_id="acc123", items=None, overall="pass"):
        # 自描述推送产物（ws_push.py --result-file，方向 1）：run_id + 逐项
        # 源/目标路径 + 三项校验值 sha256/bytes/context + overall
        d = {
            "run_id": run_id,
            "endpoint": "10.9.9.9:5555",
            "items": items if items is not None else
                     [{"module": "m1", "source": "/out/vendor/bin/app1",
                       "dst": "/vendor/bin/app1", "push_ok": True,
                       "checks": {"sha256": "pass", "bytes": "pass",
                                  "context": "pass"}}],
            "reboot": None,
            "overall": overall,
        }
    ...
    def test_mode_a_normal(self):
        ...
        with redirect_stdout(buf):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 ...
                                 "--selfcheck", "pytest_rc=0 refs_rc=0 config_rc=0 contract_rc=0 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 discipline_rc=0 scan_rc=0 ruff_rc=0 host_rc=0 metrics_rc=0 opencode_rc=0 quotepath_rc=0 known_issues_rc=0 commit_coverage_rc=0 lcview_events_rc=0 | 120 passed, 2 skipped in 5.0s"])
    ...
    _RC = ("pytest_rc=0 refs_rc=0 config_rc=0 contract_rc=0 pyenv_rc=0 "
           "ioctl_rc=0 manifest_rc=0 discipline_rc=0 scan_rc=0 "
           "ruff_rc=0 host_rc=0 metrics_rc=0 opencode_rc=0 quotepath_rc=0 known_issues_rc=0 commit_coverage_rc=0 lcview_events_rc=0")
    ...
    def test_selfcheck_failed_nonzero_rejected(self):
        ...
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "skip", "--build", "skip",
                                 "--board", "skip", "--summary", "s",
                                 "--selfcheck", "pytest_rc=0 refs_rc=0 config_rc=0 contract_rc=0 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 discipline_rc=0 scan_rc=0 ruff_rc=0 host_rc=0 metrics_rc=0 opencode_rc=0 quotepath_rc=0 known_issues_rc=0 commit_coverage_rc=0 lcview_events_rc=0 | 1 failed, 119 passed, 2 skipped in 5.0s"])
        self.assertEqual(rc, 2)
    ...
    def test_selfcheck_missing_skipped_rejected(self):
        ...
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "skip", "--build", "skip",
                                 "--board", "skip", "--summary", "s",
                                 "--selfcheck", "pytest_rc=0 refs_rc=0 config_rc=0 contract_rc=0 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 discipline_rc=0 scan_rc=0 ruff_rc=0 host_rc=0 metrics_rc=0 opencode_rc=0 quotepath_rc=0 known_issues_rc=0 commit_coverage_rc=0 lcview_events_rc=0 | 120 passed in 5.0s"])
        self.assertEqual(rc, 2)
    ...
    def test_selfcheck_config_rc_nonzero_rejected(self):
        ...
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "skip", "--build", "skip",
                                 "--board", "skip", "--summary", "s",
                                 "--selfcheck", "pytest_rc=0 refs_rc=0 config_rc=1 contract_rc=0 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 discipline_rc=0 scan_rc=0 ruff_rc=0 host_rc=0 metrics_rc=0 opencode_rc=0 quotepath_rc=0 known_issues_rc=0 commit_coverage_rc=0 lcview_events_rc=0 | "
                                 "120 passed, 2 skipped in 5.0s"])
        self.assertEqual(rc, 2)
    ...
    def test_selfcheck_contract_rc_nonzero_rejected(self):
        ...
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "skip", "--build", "skip",
                                 "--board", "skip", "--summary", "s",
                                 "--selfcheck", "pytest_rc=0 refs_rc=0 config_rc=0 contract_rc=1 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 discipline_rc=0 scan_rc=0 ruff_rc=0 host_rc=0 metrics_rc=0 opencode_rc=0 quotepath_rc=0 known_issues_rc=0 commit_coverage_rc=0 lcview_events_rc=0 | "
                                 "120 passed, 2 skipped in 5.0s"])
        self.assertEqual(rc, 2)
        self.assertIn("非零退出码", err.getvalue())
    ...
    def test_selfcheck_pytest_rc_nonzero_rejected(self):
        ...
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "skip", "--build", "skip",
                                 "--board", "skip", "--summary", "s",
                                 "--selfcheck", "pytest_rc=1 refs_rc=0 config_rc=0 contract_rc=0 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 discipline_rc=0 scan_rc=0 ruff_rc=0 host_rc=0 metrics_rc=0 opencode_rc=0 quotepath_rc=0 known_issues_rc=0 commit_coverage_rc=0 lcview_events_rc=0 | "
    ...
    def test_selfcheck_refs_rc_nonzero_rejected(self):
        ...
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "skip", "--build", "skip",
                                 "--board", "skip", "--summary", "s",
                                 "--selfcheck", "pytest_rc=0 refs_rc=1 config_rc=0 contract_rc=0 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 discipline_rc=0 scan_rc=0 ruff_rc=0 host_rc=0 metrics_rc=0 opencode_rc=0 quotepath_rc=0 known_issues_rc=0 commit_coverage_rc=0 lcview_events_rc=0 | "
    ...
    def _assert_rc_nonzero_rejected(self, rc_key):
        ...
        rcs = {k: "0" for k in
               ("pytest_rc", "refs_rc", "config_rc", "contract_rc",
                ...
                "commit_coverage_rc", "lcview_events_rc")}
        rcs[rc_key] = "1"
    ...
    def test_selfcheck_quotepath_rc_nonzero_rejected(self):
        ...
        # 未带 core.quotepath=false（quotepath_rc=1）须拒写收据
        self._assert_rc_nonzero_rejected("quotepath_rc")

    def test_selfcheck_known_issues_rc_nonzero_rejected(self):
        # 死 rc 判红（CDP 2026-09-11 批次方向 2）：baseline-status.yaml 引用
        # 的 KI 编号无对应记录文件（known_issues_rc=1，悬空引用）须拒写收据
        self._assert_rc_nonzero_rejected("known_issues_rc")

    def test_selfcheck_commit_coverage_rc_nonzero_rejected(self):
        # 死 rc 判红（批次 133b55812a81 方向 3）：自最近 baseline 起非 meta
        # 提交无收据覆盖（commit_coverage_rc=1）须拒写收据
        self._assert_rc_nonzero_rejected("commit_coverage_rc")

    def test_selfcheck_lcview_events_rc_nonzero_rejected(self):
        # 死 rc 判红（方向 5）：lcview 事件 schema 与内核发射点 id/字段序
        # 漂移（lcview_events_rc=1）须拒写收据
        self._assert_rc_nonzero_rejected("lcview_events_rc")
    ...
    def test_selfcheck_contradictory_refs_text_rejected(self):
        ...
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "skip", "--build", "skip",
                                 "--board", "skip", "--summary", "s",
                                 "--selfcheck", "pytest_rc=0 refs_rc=0 config_rc=0 contract_rc=0 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 discipline_rc=0 scan_rc=0 ruff_rc=0 host_rc=0 metrics_rc=0 opencode_rc=0 quotepath_rc=0 known_issues_rc=0 commit_coverage_rc=0 lcview_events_rc=0 | "
                                 "531 passed in 27.9s | skipped=0 | "
                                 "==== 共 3 处悬空引用（exit 1）===="])
        self.assertEqual(rc, 2)
    ...
    def test_mode_b_board_fail_relaxes_rc_requirement(self):
        ...
        with redirect_stdout(buf):
            rc = ws_report.main(["--target", "1a2b3c4d5e6f",
                                 ...
                                 "--selfcheck", "pytest_rc=1 refs_rc=0 config_rc=0 contract_rc=0 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 discipline_rc=0 scan_rc=0 ruff_rc=0 host_rc=0 metrics_rc=0 opencode_rc=0 quotepath_rc=0 known_issues_rc=0 commit_coverage_rc=0 lcview_events_rc=0 | 1 failed, 119 passed, 2 skipped in 5.0s"])
        self.assertEqual(rc, 0)
        details = [f for f in self._dir.glob("*.md") if f.name != "trend.md"]
        self.assertEqual(len(details), 1)
        content = details[0].read_text(encoding="utf-8")
    ...
    def test_mode_b_board_pass_still_requires_rc_all_zero(self):
        ...
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--target", "1a2b3c4d5e6f",
                                 ...
                                 "--board", "pass", "--summary", "上板通过",
                                 "--case", "lcview-liveness",
                                 "--selfcheck", "pytest_rc=1 refs_rc=0 config_rc=0 contract_rc=0 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 discipline_rc=0 scan_rc=0 ruff_rc=0 host_rc=0 metrics_rc=0 opencode_rc=0 quotepath_rc=0 known_issues_rc=0 commit_coverage_rc=0 lcview_events_rc=0 | 120 passed, 2 skipped in 5.0s",
                                 "--acceptance-file", self._write_acc(),
    ...
    def _mk_manual_git(self, commits=("构建(baseline): 基线", "修复(harness): 修复")):
        ...
        返回 (repo_path, base_sha, head_sha)。commits[0] 为最近 promoted
        baseline source_commit，commits[1:] 为其后提交（非 meta 须被收据覆盖）。
        """
        repo = Path(self._tmp.name) / "mrepo"
        ...
        subprocess.run(["git", "-C", str(repo), "config", "user.name",
                        "t"], check=True)
        shas = []
        for i, title in enumerate(commits):
            p = repo / f"f{i}.py"
            p.write_text("x\n", encoding="utf-8")
            subprocess.run(["git", "-C", str(repo), "add", "--", f"f{i}.py"],
                           check=True)
        ...
        (cfg / "baseline-status.yaml").write_text(
            f"baselines:\n- baseline_id: BL-A\n  status: promoted\n"
            f"  source_commit: {shas[0]}\n", encoding="utf-8")
        return str(repo), shas[0], shas[-1]

    def test_mode_m_manual_backfills_commit_scope(self):
        # 模式 M：--manual <base>..<head> 生成 manual 收据并回填 commit_scope
        # （区间非 meta 提交改动面），收据落盘且 commit_scope 覆盖 f1.py
        ...
        self.assertEqual(len(details), 1)
        content = details[0].read_text(encoding="utf-8")
        self.assertIn("- batch_id: manual-", content)
        self.assertIn("- batch_base: %s" % base, content)
        self.assertIn("f1.py", content)  # commit_scope 含非 meta 提交改动文件

    def test_mode_m_without_body_rejected(self):
        # 模式 M 必须传 --body（逐项三态自报，LOCAL-DEV-002）
        repo, base, head = self._mk_manual_git()
        os.environ["CDP_PROJECT_ROOT"] = repo
        self.addCleanup(lambda: os.environ.update(
            {"CDP_PROJECT_ROOT": self._tmp.name}))
        ...
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--manual", f"{base}..{head}",
                                 "--result", "skip", "--build", "skip",
                                 "--board", "skip", "--summary", "x",
                                 "--selfcheck", "pytest_rc=0 refs_rc=0 config_rc=0 contract_rc=0 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 discipline_rc=0 scan_rc=0 ruff_rc=0 host_rc=0 metrics_rc=0 opencode_rc=0 quotepath_rc=0 known_issues_rc=0 commit_coverage_rc=0 lcview_events_rc=0 | 120 passed, 2 skipped in 5.0s"])
        self.assertEqual(rc, 2)
        self.assertIn("必须传 --body", err.getvalue())

    def test_mode_m_without_selfcheck_rejected(self):
        # 模式 M 必须带 selfcheck（LOCAL-DEV-003）
        repo, base, head = self._mk_manual_git()
        os.environ["CDP_PROJECT_ROOT"] = repo
        self.addCleanup(lambda: os.environ.update(
            {"CDP_PROJECT_ROOT": self._tmp.name}))
    ...
    def test_mode_m_git_failure_rejected(self):
        # 模式 M：git rev-list 失败（无法回填 commit_scope）→ fail-closed 拒写
        repo, base, head = self._mk_manual_git()
        os.environ["CDP_PROJECT_ROOT"] = repo
        self.addCleanup(lambda: os.environ.update(
            {"CDP_PROJECT_ROOT": self._tmp.name}))
    ...
    def test_mode_m_rc1_with_real_gap_exempted(self):
        ...
        with redirect_stdout(buf):
            rc = ws_report.main(["--manual", f"{base}..{head}",
                                 "--result", "skip", "--build", "skip",
                                 "--board", "skip", "--summary", "直连开发",
                                 "--selfcheck", "pytest_rc=0 refs_rc=0 config_rc=0 contract_rc=0 pyenv_rc=0 ioctl_rc=0 manifest_rc=0 discipline_rc=0 scan_rc=0 ruff_rc=0 host_rc=0 metrics_rc=0 opencode_rc=0 quotepath_rc=0 known_issues_rc=0 commit_coverage_rc=1 lcview_events_rc=0 | 120 passed, 2 skipped in 5.0s",
                                 "--body", body])
        self.assertEqual(rc, 0, buf.getvalue())
        details = [f for f in (Path(repo) / "data" / "verify-results")
                   .glob("*.md") if f.name != "trend.md"]
    ...
    def test_pass_legacy_acceptance_rejected(self):
        # 方向 1：PASS 只接受 --acceptance-file，--acceptance 直传 JSON 已废弃
        batch = self._write(VALID_S, ".cdp")
        body = self._write("## 现场\n")
        err = io.StringIO()
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 ...
                                 '{"overall": "pass", "items": []}'])
        self.assertEqual(rc, 2)
        self.assertIn("已改为只接受 --acceptance-file", err.getvalue())
        self.assertFalse(self._dir.exists())

    def test_pass_acceptance_file_invalid_json_rejected(self):
        # result=pass 而 --acceptance-file 非合法 JSON（如手填 "ok"）→ 返 2 拒写
        batch = self._write(VALID_S, ".cdp")
        body = self._write("## 现场\n")
        bad = self._write("手填 ok", ".json")
        err = io.StringIO()
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "pass", "--build", "pass",
    ...
    def test_pass_acceptance_file_overall_fail_rejected(self):
        # result=pass 而验收产物 overall=fail → 返 2 拒写（失败验收不得过 promote）
        batch = self._write(VALID_S, ".cdp")
        body = self._write("## 现场\n")
        acc = self._write_acc(overall="fail")
        err = io.StringIO()
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "pass", "--build", "pass",
    ...
    def test_pass_acceptance_file_fail_item_rejected(self):
        # overall=pass 但 items 含 fail 项（自相矛盾）→ 返 2 拒写
        batch = self._write(VALID_S, ".cdp")
        body = self._write("## 现场\n")
        acc = self._write_acc(items=[{"status": "fail"}])
        err = io.StringIO()
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "pass", "--build", "pass",
    ...
    def test_pass_acceptance_file_valid_singlelined(self):
        # 自描述验收产物 + 单测产物（run_id 一致）→ exit 0，且单行化落盘
        # （header 逐行 key-value，多行 JSON 会被 from_text 截断）
        batch = self._write(VALID_S, ".cdp")
        body = self._write("## 现场\n")
        acc = self._write_acc(run_id="runabc123")
        ut = self._write_ut(run_id="runabc123")
        buf = io.StringIO()
        with redirect_stdout(buf):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "pass", "--build", "pass",
    ...
    def test_pass_acceptance_file_array_rejected(self):
        # 方向 1：PASS 只接受自描述验收产物（JSON 对象），历史数组格式不再接受
        batch = self._write(VALID_S, ".cdp")
        body = self._write("## 现场\n")
        arr = self._write('[{"tag": "svc:x", "status": "pass"}]', ".json")
        err = io.StringIO()
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "pass", "--build", "pass",
                                 ...
                                 "--unit-test-file", self._write_ut(), "--push-file", self._write_push()])
        self.assertEqual(rc, 2)
        self.assertIn("须为 JSON 对象", err.getvalue())
        self.assertFalse(self._dir.exists())

    def test_pass_acceptance_file_missing_run_id_rejected(self):
    ...
    def test_pass_acceptance_file_stale_monotonic_rejected(self):
        # 方向 3：单调时间异常（start>=end）→ 拒 PASS（不用固定墙钟，用单调时间表达）
        batch = self._write(VALID_S, ".cdp")
        body = self._write("## 现场\n")
        acc = self._write_acc(start=2.0, end=1.0)
        err = io.StringIO()
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "pass", "--build", "pass",
                                 ...
                                 "--unit-test-file", self._write_ut(), "--push-file", self._write_push()])
        self.assertEqual(rc, 2)
        self.assertIn("单调时间异常", err.getvalue())
        self.assertFalse(self._dir.exists())

    def test_pass_acceptance_file_summary_mismatch_rejected(self):
        # 方向 1：-sv 批（验收文本非「无」）产物输入摘要与本批不一致 → 拒 PASS
...
""", ".cdp")
        body = self._write("## 现场\n")
        acc = self._write_acc(summary="svc:other_tag")
        err = io.StringIO()
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 "--result", "pass", "--build", "pass",
                                 ...
                                 "--case", "lcview-liveness"])
        self.assertEqual(rc, 2)
        self.assertIn("输入摘要与本批验收文本不一致", err.getvalue())
        self.assertFalse(self._dir.exists())

    def test_pass_without_unit_test_file_rejected(self):
        # 方向 2：PASS 缺 --unit-test-file → 返 2 拒写
        batch = self._write(VALID_S, ".cdp")
        body = self._write("## 现场\n")
        err = io.StringIO()
        with contextlib.redirect_stderr(err):
    ...
    def test_pass_unit_test_run_id_mismatch_rejected(self):
        # 方向 2：单测产物 run_id 与验收产物不一致（非同批）→ 拒 PASS
        batch = self._write(VALID_S, ".cdp")
        body = self._write("## 现场\n")
        err = io.StringIO()
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 ...
                                 "--push-file", self._write_push()])
        self.assertEqual(rc, 2)
        self.assertIn("run_id 与验收产物不一致", err.getvalue())
        self.assertFalse(self._dir.exists())

    def test_pass_unit_test_not_green_rejected(self):
        # 方向 2：单测 target 非全绿（failed>0 / rc=1）→ 拒 PASS
        batch = self._write(VALID_S, ".cdp")
        body = self._write("## 现场\n")
        for targets in ([{"name": "t1", "rc": 0, "tests": 42, "failed": 1}],
                        [{"name": "t1", "rc": 1, "tests": 42, "failed": 0}]):
            err = io.StringIO()
            with contextlib.redirect_stderr(err):
                rc = ws_report.main(["--batch-file", batch, "--body", body,
                                     "--result", "pass", "--build", "pass",
    ...
    def test_pass_without_push_file_rejected(self):
        ...
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 ...
                                 "--unit-test-file", self._write_ut()])
        self.assertEqual(rc, 2)
        self.assertIn("必须传 --push-file", err.getvalue())
        self.assertFalse(self._dir.exists())

    def test_pass_push_file_invalid_json_rejected(self):
        # 方向 1：--push-file 非合法 JSON → 返 2 拒写
        batch = self._write(VALID_S, ".cdp")
        body = self._write("## 现场\n")
        bad = self._write("手填 ok", ".json")
        err = io.StringIO()
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 ...
                                 "--push-file", bad])
        self.assertEqual(rc, 2)
        self.assertIn("--push-file 须为合法 JSON", err.getvalue())
        self.assertFalse(self._dir.exists())

    def test_pass_push_file_missing_run_id_rejected(self):
        # 方向 1：推送产物缺 run_id → 拒 PASS
        batch = self._write(VALID_S, ".cdp")
        body = self._write("## 现场\n")
        push = self._write(json.dumps({"items": [], "overall": "pass"}),
                           ".json")
        err = io.StringIO()
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 ...
                                 "--push-file", push])
        self.assertEqual(rc, 2)
        self.assertIn("--push-file 缺 run_id", err.getvalue())
        self.assertFalse(self._dir.exists())

    def test_pass_push_file_run_id_mismatch_rejected(self):
        # 方向 1：推送产物 run_id 与验收产物不一致（非同批）→ 拒 PASS
        batch = self._write(VALID_S, ".cdp")
        body = self._write("## 现场\n")
        err = io.StringIO()
        with contextlib.redirect_stderr(err):
            rc = ws_report.main(["--batch-file", batch, "--body", body,
                                 ...
                                 "--unit-test-file", self._write_ut(),
                                 "--push-file",
                                 self._write_push(run_id="other456")])
        self.assertEqual(rc, 2)
        self.assertIn("--push-file run_id 与验收产物不一致", err.getvalue())
        self.assertFalse(self._dir.exists())

    def test_pass_push_file_empty_items_rejected(self):
        # 方向 1：items 为空（无任何推送项证据）→ 拒 PASS
        batch = self._write(VALID_S, ".cdp")
        body = self._write("## 现场\n")
        err = io.StringIO()
        with contextlib.redirect_stderr(err):
        ...
        self.assertEqual(rc, 2)
        self.assertIn("items 为空", err.getvalue())
        self.assertFalse(self._dir.exists())

    def test_pass_push_file_checks_not_green_rejected(self):
        # 方向 1：任一项 sha256/bytes/context 三项校验值有 fail → 拒 PASS
        batch = self._write(VALID_S, ".cdp")
        body = self._write("## 现场\n")
        for bad_key in ("sha256", "bytes", "context"):
            items = [{"module": "m1", "source": "/out/x", "dst": "/vendor/bin/x",
                      "push_ok": True,
                      "checks": {"sha256": "pass", "bytes": "pass",
                                 "context": "pass"}}]
            items[0]["checks"][bad_key] = "fail"
            err = io.StringIO()
            with contextlib.redirect_stderr(err):
                rc = ws_report.main(["--batch-file", batch, "--body", body,
                                     "--result", "pass", "--build", "pass",
                                     ...
                                     "--unit-test-file", self._write_ut(),
                                     "--push-file",
                                     self._write_push(items=items)])
            self.assertEqual(rc, 2, bad_key)
            self.assertIn("校验值非全绿", err.getvalue(), bad_key)
        self.assertFalse(self._dir.exists())

    def test_pass_push_file_overall_fail_rejected(self):
        # 方向 1：推送产物 overall 非 pass（推送存在失败/重启门禁红）→ 拒 PASS
        batch = self._write(VALID_S, ".cdp")
        body = self._write("## 现场\n")
        err = io.StringIO()
        with contextlib.redirect_stderr(err):
    ...
    def test_selfcheck_four_rc_all_zero_passes(self):
        # 六 rc（pytest/refs/config/contract/pyenv/ioctl，REQUIRED_RC_KEYS）
        # 全零 → 正常落盘
        batch = self._write(VALID_S, ".cdp")
        body = self._write("## 现场\n")
        buf = io.StringIO()
        with redirect_stdout(buf):
            rc = ws_report.main(["--batch-file", batch, "--result", "skip",
                                 ...
                                 "contract_rc=0 | OK: contract 检查通过，无违规。 | "
                                 "pyenv_rc=0 | python=3.12 OK | "
                                 "ioctl_rc=0 | OK: 一致 | "
                                 "manifest_rc=0 discipline_rc=0 scan_rc=0 ruff_rc=0 host_rc=0 metrics_rc=0 opencode_rc=0 quotepath_rc=0 known_issues_rc=0 commit_coverage_rc=0 lcview_events_rc=0 | manifest.yaml 无变化"])
        self.assertEqual(rc, 0)
    ...
    def test_device_dirty_auto_from_acceptance_rejects_pass(self):
        # 方向 5：验收产物 device_dirty=true（teardown 恢复失败）+ result=pass
        # → 拒落收据（设备态不可信的验证结果不得记 pass；fail/skip 仍可落）
        batch = self._write(VALID_S, ".cdp")
        body = self._write("## 现场\n")
