...
import sync_workspace_to_code as sw


class TestGenManifestOnly(unittest.TestCase):
    ...
    def test_check_only_no_write_when_absent(self):
        # --gen-manifest-only --check-only：manifest 不存在时不写盘
        root = self._make_patch_root({"aosp/new/vendor/x/foo.h": "//x"})
        sw.generate_manifest(root, check_only=True,
    ...
    def test_gen_writes_entries_with_source_map(self):
        ...
        sw.generate_manifest(root, check_only=False,
                             kernel_deletions=[], aosp_deletions=[])
