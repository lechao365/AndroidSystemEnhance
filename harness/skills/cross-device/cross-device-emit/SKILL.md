...
## 复盘判据（耗时异常标定）
- 复盘时按 verify_mode 分模式（none/board）读取 data/verify-results/trend.md
  行尾 timing JSON（elapsed_s + segs 段名→秒数映射），取各段历批中位数
- 当前批某段耗时超过该段同模式中位数 2 倍即标异常，复盘/下批据此给针对性
  修复方向（如 edit 段异常高指向编辑低效、verify_build 异常高指向编译瓶颈）
