...
- 用户执行完归档（sync-workspace-to-code）后
- 用户提到"更新文档""文档同步""code 变了"
- `code/rpi5/` 相对 git HEAD 有未同步到文档的变动

## Preconditions（前置条件）

- `code/rpi5/manifest.yaml` 存在（首次未归档时停下提示用户先执行 `/sync-workspace-to-code`）
...
| 场景 | 处理 |
|------|------|
| manifest 缺失（首次未归档） | 停下提示用户先执行 `/sync-workspace-to-code` |
...
```bash
python3 harness/skills/sync-code-to-doc/sync_code_to_doc.py              # 生成变动报告
...
`code/` 中 modified 类只有 `.diff`、无全量上下文。通过 `code/rpi5/manifest.yaml`（由 sync-workspace-to-code 生成）拿到 patch↔workspace 映射，去 `~/workspace/` 读全量源码：
...
> **边界**：若 manifest 缺失（首次未归档），停下提示用户先执行 `/sync-workspace-to-code`。
...
`code/rpi5/README.md` 文件映射表的更新仍走 sync-workspace-to-code 末尾提示，不纳入本流程。
