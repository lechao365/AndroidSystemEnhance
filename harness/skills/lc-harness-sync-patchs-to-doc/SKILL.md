---
name: lc-harness-sync-patchs-to-doc
...
- 用户执行完归档（lc-harness-sync-code-to-patchs）后
- 用户提到"更新文档""文档同步""code 变了"
- `code/rpi5/` 相对 git HEAD 有未同步到文档的变动

## Preconditions（前置条件）

- `code/rpi5/manifest.yaml` 存在（首次未归档时停下提示用户先执行 `/lc-harness-sync-code-to-patchs`）
...
脚本基于 git HEAD 对比 `code/rpi5/`，按目录分组（`kernel/modified`、`kernel/new`、`aosp/modified`、`aosp/new`、`others`）输出变动类型（A/M/D/R）和行数统计。加 `--full-diff` 在报告末尾追加 `git diff HEAD` 完整正文。
...
- 落盘后自动一致性自检报告：文档章节结构 / 代码路径合规 / 行号锚点有效性 / 交叉引用可达 / 形态 D 盲区

## Failure / recovery（失败/恢复）

| 场景 | 处理 |
|------|------|
| manifest 缺失（首次未归档） | 停下提示用户先执行 `/lc-harness-sync-code-to-patchs` |
...
```bash
python3 harness/skills/lc-harness-sync-patchs-to-doc/lc_harness_sync_patchs_to_doc.py              # 生成变动报告
python3 harness/skills/lc-harness-sync-patchs-to-doc/lc_harness_sync_patchs_to_doc.py --full-diff  # 报告 + 完整 diff 正文（AI 零往返）
python3 harness/skills/lc-harness-sync-patchs-to-doc/lc_harness_sync_patchs_to_doc.py --check-only  # 仅检查，不输出提示
```
...
- **精确匹配**：`**/LcView/**`→`docs/01-打点增强/`、`**/LcIod/**`→`docs/02-IO增强/`、`others/usb-*`→`docs/02-IO增强/` 等（首条命中即归属）
...
`code/` 中 modified 类只有 `.diff`、无全量上下文。通过 `code/rpi5/manifest.yaml`（由 lc-harness-sync-code-to-patchs 生成）拿到 patch↔workspace 映射，去 `~/workspace/` 读全量源码：
...
> **边界**：若 manifest 缺失（首次未归档），停下提示用户先执行 `/lc-harness-sync-code-to-patchs`。
...
`code/rpi5/README.md` 文件映射表的更新仍走 lc-harness-sync-code-to-patchs 末尾提示，不纳入本流程。
