...
class TestSelfcheckWorkflow(unittest.TestCase):
    ...
    def test_runs_selfcheck_only_with_degraded_pointer(self):
        ...
        # （无打点指针降级路径）。runner 上下文仅 step 级可用：job 级 env
        # 用 ${{ runner.temp }} 会致工作流启动失败（89 run 零 job），step 级
        # env 用 ${{ runner.temp }} 表达式正确（Actions env 不做 shell 展开，
        # $RUNNER_TEMP 字面量会在检出树建同名目录污染检出树）
        job = self.doc["jobs"]["selfcheck"]
        steps = job["steps"]
        run_steps = [s.get("run", "") for s in steps]
        joined = "\n".join(run_steps)
        self.assertIn("selfcheck.py", joined)
        # job 级 env 不得用表达式上下文（runner.temp 在 job 级不可用）
        self.assertNotIn("runner.temp", str(job.get("env", {})))
        # 自检 step 级 env 用 ${{ runner.temp }} 表达式（step 内可用，
        # 求值为真实临时路径；字面量 $RUNNER_TEMP 是 bug）
