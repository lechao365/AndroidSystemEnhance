...
# --- 脚本目录 + 精简公共库（自包含，替代 LcHarness shell 库）---------------
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ARTIFACT_DIR="${SCRIPT_DIR}/../log/mk_rpi5_full_image"
...
harness_tmp_file() { mktemp "${TMPDIR:-/tmp}/${1:-tmp}.XXXXXX"; }
artifact_register() {
    local src="$1" name="${2:-$(basename "$1")}"
    mkdir -p "$ARTIFACT_DIR"
    if cp "$src" "$ARTIFACT_DIR/$name" 2>/dev/null; then
        log_info "制品归档: $ARTIFACT_DIR/$name"
    else
        log_warn "制品归档失败: $src"
    fi
}
...
AOSP_ROOT="${AOSP_ROOT:-}"  # 由用户环境变量设置
KERNEL_SRC="${KERNEL_SRC:-}"  # 由用户环境变量设置
KERNEL_OUT="${KERNEL_OUT:-}"
KERNEL_DEST="${KERNEL_DEST:-${AOSP_ROOT}/device/brcm/rpi5-kernel}"
WINDOWS_IMG_DIR="${WINDOWS_IMG_DIR:-}"
