...
> 源码实现：`readOnce`（[main_loop.cpp:58](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/main_loop.cpp#L58)），`waitAndRead` 返回 `-1` 视为致命（[DeviceReader.h:36](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/DeviceReader.h#L36)）。
...
- **LCV-04**：FLOAT 字段 NaN/Inf 输出 `null`（JSON 标准不定义 NaN/Infinity 字面量，直写会产生非法 JSON，[`FileWriter.cpp:342`](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/FileWriter.cpp#L342)）
- **LCV-20**：FLOAT 精度 `setprecision(9)`（单精度有效数字 7~9 位，默认 6 位精度丢尾数，[`FileWriter.cpp:347`](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/FileWriter.cpp#L347)）
- **LCV-05**：STRING 字段非法 UTF-8 字节逐字节 `\u00XX` 转义、合法 UTF-8 序列整体直传（USB 描述符来自设备固件可含任意字节，保输出行恒为合法 JSONL，[`FileWriter.cpp:281-306`](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/FileWriter.cpp#L281)）
...
| 每条记录写入后立即 flush | `FileWriter::writeLineFlush()` 内 `stream.flush()` | [`FileWriter.cpp:453`](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/FileWriter.cpp#L453) |
| 空缓冲不产生写放大 | `shouldFlushBatch(buffered==0)` 返回 false，空批不 flush 攒包 | [`batch_parser.cpp:114`](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/batch_parser.cpp#L114) |
| 文件轮转不丢数据 | 轮转时先关闭旧文件再打开新文件，追加模式写入 | [`FileWriter.cpp:691`](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/FileWriter.cpp#L691) |
...
| `DeviceReader.h` | 设备读取抽象声明 | `open()` / `waitAndRead()` / `getOverrun()` / `getTotalRecords()` / `ioctlErr()` / `eofCount()` / `close()` | 82 |
| `DeviceReader.cpp` | EpollDeviceReader 生产实现 | `open()` / `waitAndRead()` | 162 |
| `batch_parser.h` | 可测函数声明 | `parseBatch()` / `loadSchemaWithRetry()` / `shouldFlushBatch()` | 58 |
| `batch_parser.cpp` | 攒包解析、schema 重试、flush 判定 | `parseBatch()` / `loadSchemaWithRetry()` / `shouldFlushBatch()` | 119 |
...
| `FileWriter.h` | 文件写入器声明 | `writeRecord()` / `writeInvalid()` / `checkRotation()` / `enforceRetention()` | 166 |
| `FileWriter.cpp` | JSONL 格式化、滚动、容量管理 | `formatJsonLine()` / `writeLineFlush()` / `nextSeqFor()` / `enforceRetention()` | 842 |
...
```c
// FileWriter.cpp:453（writeLineFlush 内 flush 段）
fs.stream << line;
fs.stream.flush();
if (fs.stream.fail()) {
...
| `waitAndRead(buf, offset, cap, timeoutMs)` | `uint8_t*` + `size_t` + `size_t` + `int` | `ssize_t` | `>0` 数据 / `0` 超时（含 EINTR·EAGAIN） / `-1` 致命（EOF 置 `errno=ENODEV` 返 -1，不再返 0 伪装"本次无数据"） |
...
| `writeInvalid(data, len, reason)` | `uint8_t*` + `size_t` + `string` | 无 | 写入非法记录到 `invalid_records.log`（reason 经 jsonEscapeString 转义；写失败 clear+reopen+retry 自愈，重开前 `rollbackFileTo` 截断回写前偏移防残留半行粘连（LCV-07），仍失败计 `invalidWriteFailed` 并清粘滞；超 10MB 触发轮转，LCV-01）|
...
| 心跳间隔 | 30 秒 | `main_loop.cpp:223` | 距上次满 30s 输出一次心跳日志（时间驱动，与负载解耦）|
...
| invalid 文件轮转阈值 | 10MB | `FileWriterConfig::maxInvalidFileSizeMb`（FileWriter.h:40） | `invalid_records.log` 超限轮转（LCV-01） |
