...
> 源码引用：`loadSchemaWithRetry()`（[batch_parser.cpp:96](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/batch_parser.cpp#L96)），调用点 [lechao_lcview.cpp:215](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/lechao_lcview.cpp#L215)。
...
> 源码实现见 [关键设计「设备打开重试」](#设备打开重试--启动顺序解耦)（[lechao_lcview.cpp:236](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/lechao_lcview.cpp#L236)）。
...
| 模块 | 职责 | 关键抽象 |
|------|------|---------|
| `lechao_lcview.cpp` | 设备直读、主循环、信号处理 | `main()` — [lechao_lcview.cpp:196](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/lechao_lcview.cpp#L196) |
...
| 不变量 | 保证机制 | 代码位置 |
|--------|---------|---------|
| Schema 加载成功后主循环期间只读 | `loadSchemaWithRetry()` 成功后 `mSchemaMap` 不修改 | [`lechao_lcview.cpp:215`](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/lechao_lcview.cpp#L215) |
| 每条记录写入后立即 flush | `FileWriter::writeLineFlush()` 内 `stream.flush()` | [`FileWriter.cpp:356`](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/FileWriter.cpp#L356) |
...
| 文件轮转不丢数据 | 轮转时先关闭旧文件再打开新文件，追加模式写入 | [`FileWriter.cpp:470`](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/FileWriter.cpp#L470) |
...
| 文件 | 职责 | 关键函数 | 行数 |
|------|------|---------|------|
| `lechao_lcview.cpp` | 设备直读、主循环、信号处理 | `main()` / `runMainLoop()` / `readOnce()` / `flushSegment()` / `emitHeartbeat()` | 254 |
...
| `FileWriter.h` | 文件写入器声明 | `writeRecord()` / `writeInvalid()` / `checkRotation()` / `enforceRetention()` | 146 |
| `FileWriter.cpp` | JSONL 格式化、滚动、容量管理 | `formatJsonLine()` / `writeLineFlush()` / `nextSeqFor()` / `enforceRetention()` | 612 |
...
```c
// flushSegment（lechao_lcview.cpp:124-150）：readOnce → emitHeartbeat(每30 loop) → flushSegment
...
直读主循环每轮：`readOnce` 读内核 → 每 30 loop `emitHeartbeat` → `flushSegment` 攒包写盘；epoll 内部阻塞机制保证 CPU 零开销等待。
...
```c
// FileWriter.cpp:356（writeLineFlush 内 flush 段）
fs.stream << line;
fs.stream.flush();
if (fs.stream.fail()) {
...
| 常量 | 值 | 定义位置 | 说明 |
|------|-----|---------|------|
| Schema 重试次数 | 30 | `batch_parser.cpp:96`（调用点 `lechao_lcview.cpp:215`） | 每次 500ms，最多 15 秒 |
| 设备打开重试次数 | 1200 | `lechao_lcview.cpp:237` | 每次 100ms，最多 120 秒 |
| 心跳间隔 | 30 次循环 | `lechao_lcview.cpp:177` | 每 30 次循环输出一次心跳日志 |
