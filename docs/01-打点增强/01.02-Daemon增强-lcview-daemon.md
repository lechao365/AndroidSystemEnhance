...
> 源码引用：`loadSchemaWithRetry()`（[batch_parser.cpp:101](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/batch_parser.cpp#L101)），调用点 [lechao_lcview.cpp:255](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/lechao_lcview.cpp#L255)。
...
> 源码实现见 [关键设计「设备打开重试」](#设备打开重试--启动顺序解耦)（[lechao_lcview.cpp:276](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/lechao_lcview.cpp#L276)）。
...
| 模块 | 职责 | 关键抽象 |
|------|------|---------|
| `lechao_lcview.cpp` | 设备直读、主循环、信号处理 | `main()` — [lechao_lcview.cpp:236](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/lechao_lcview.cpp#L236) |
...
| 不变量 | 保证机制 | 代码位置 |
|--------|---------|---------|
| Schema 加载成功后主循环期间只读 | `loadSchemaWithRetry()` 成功后 `mSchemaMap` 不修改 | [`lechao_lcview.cpp:255`](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lcview/daemon/lechao_lcview.cpp#L255) |
...
| 常量 | 值 | 定义位置 | 说明 |
|------|-----|---------|------|
| Schema 重试次数 | 30 | `batch_parser.cpp:101`（调用点 `lechao_lcview.cpp:255`） | 每次 500ms，最多 15 秒 |
| 设备打开重试次数 | 1200 | `lechao_lcview.cpp:276` | 每次 100ms，最多 120 秒 |
| 心跳间隔 | 30 秒 | `lechao_lcview.cpp:193` | 距上次满 30s 输出一次心跳日志（时间驱动，与负载解耦）|
