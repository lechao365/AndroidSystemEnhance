...
| `lcview_main` | 字符设备注册、file_operations、级别过滤、模块入口 | `lcview_fops` — [lcview_main.c:248](../../code/rpi5/kernel/new/vendor/lechao/LcView/lcview_main.c#L248) |
...
> `read_buf` 分配：[`lcview_ring_init`](../../code/rpi5/kernel/new/vendor/lechao/LcView/lcview_ring.c#L133)，锁内拷贝：[`lcview_ring_read`](../../code/rpi5/kernel/new/vendor/lechao/LcView/lcview_ring.c#L449)。
...
> 驱逐实现：[`ring_evict_one`](../../code/rpi5/kernel/new/vendor/lechao/LcView/lcview_ring.c#L197)，批量逻辑：[`lcview_ring.c:293`](../../code/rpi5/kernel/new/vendor/lechao/LcView/lcview_ring.c#L293)

### 关键不变量

| 不变量 | 保证机制 | 代码位置 |
|--------|---------|---------|
| `write_pos == read_pos` 恒为"空" | 预留 1 字节不可用 | [`ring_avail_write`](../../code/rpi5/kernel/new/vendor/lechao/LcView/lcview_ring.c#L59) |
| 单实例 open | `atomic_cmpxchg` | [`lcview_main.c:99`](../../code/rpi5/kernel/new/vendor/lechao/LcView/lcview_main.c#L99) |
...
| 4B 长度前缀 wrap-around | `ring_memcpy_in/out` 分段拷贝 | [`lcview_ring.c:96`](../../code/rpi5/kernel/new/vendor/lechao/LcView/lcview_ring.c#L96) |
...
| 符号 | 说明 | 定义位置 |
|------|------|---------|
| `lcview_builder_start(event_id, level)` | 分配 builder，低于级别阈值返回 NULL | [lcview_main.c:274](../../code/rpi5/kernel/new/vendor/lechao/LcView/lcview_main.c#L274) |
...
> 初始化入口：[`lcview_init`](../../code/rpi5/kernel/new/vendor/lechao/LcView/lcview_main.c#L317)
...
`min_level` 默认 `LCVIEW_LEVEL_DEBUG(0)`（不过滤），由用户态通过 `LCVIEW_SET_LEVEL` ioctl 动态调整（[`lcview_main.c:216`](../../code/rpi5/kernel/new/vendor/lechao/LcView/lcview_main.c#L216)）。对比传统 `printk` 的"先格式化再判断 level"，此设计在 DEBUG 级别被过滤时节省了完整的 `kmalloc(4100B) + 字段序列化 + spinlock 写入` 开销。
...
读取路径的关键步骤（[`lcview_ring_read`](../../code/rpi5/kernel/new/vendor/lechao/LcView/lcview_ring.c#L350)，`lcview_ring.c:350`）：
...
[`ring_evict_one`](../../code/rpi5/kernel/new/vendor/lechao/LcView/lcview_ring.c#L197)（`lcview_ring.c:197`）读取被驱逐记录的 4B 长度前缀，推进 `read_pos`，递增 `overrun_cnt`。包含防御性处理：若长度前缀异常（0 或 > ring->size），使用保守默认值（前缀 + 记录头 = 20B）跳过，避免数据损坏导致永久性错乱。
...
[`ring_memcpy_out`](../../code/rpi5/kernel/new/vendor/lechao/LcView/lcview_ring.c#L79)（`lcview_ring.c:79`）为对称的读取版本。写入时先 `ring_memcpy_in` 写 4B 长度前缀，再 `ring_memcpy_in` 写记录数据（`lcview_ring.c:310-316`），两次调用各自独立处理回绕。`write_pos` 推进使用 `% ring->size` 取模。
...
| 不变量 | 守护机制 | 源码位置 |
|--------|---------|---------|
| 单实例 open | `atomic_cmpxchg(&device_opened, 0, 1)` — 无锁 CAS，多线程并发仅一个成功 | [`lcview_main.c:99`](../../code/rpi5/kernel/new/vendor/lechao/LcView/lcview_main.c#L99) |
...
| shutdown 顺序 | 先设 `shutdown=true` 再 `wake_up`，确保等待中的 reader 退出后才释放内存 | [`lcview_ring.c:172`](../../code/rpi5/kernel/new/vendor/lechao/LcView/lcview_ring.c#L172) |
...
8 个 Builder API 通过 `EXPORT_SYMBOL` 导出到内核符号表（[`lcview_main.c:280-298`](../../code/rpi5/kernel/new/vendor/lechao/LcView/lcview_main.c#L280)），其他模块通过 `extern` 声明或包含 `lcview_events.h` 调用：
...
构建集成通过 Kconfig/Makefile 独立配置（`Kconfig` 中 `CONFIG_LCVIEW=y`），`module_init(lcview_init)` 映射为 `device_initcall`（[`lcview_main.c:366`](../../code/rpi5/kernel/new/vendor/lechao/LcView/lcview_main.c#L366)），随 vmlinux 启动。上层 Kconfig 仅需 `source "vendor/lechao/LcView/Kconfig"` 一行即可接入。
...
> `file_operations` 定义：[`lcview_main.c:248`](../../code/rpi5/kernel/new/vendor/lechao/LcView/lcview_main.c#L248)
