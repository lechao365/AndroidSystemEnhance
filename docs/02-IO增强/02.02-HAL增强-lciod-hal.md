...
| 模块 | 职责 | 关键抽象 |
|------|------|---------|
| `hal_service.cpp` | AIDL 接口实现、设备缓存管理、字段映射 | `class IoHalImpl` — [hal_service.h:47](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lciod/hal/hal_service.h#L47) |
| `device_io.cpp` | 底层 IO 封装（glob/ioctl/poll/read） | `list_devices()` / `read_event()` — [device_io.cpp:31](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lciod/hal/device_io.cpp#L31) |
| `vendor_lechao_usbd-ioctl.h` | 内核-用户态共享 ABI | `struct vendor_lechao_usbd_stats` — [vendor_lechao_usbd-ioctl.h:55](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lciod/hal/vendor_lechao_usbd-ioctl.h#L55) |
...
note bottom of MAP: LCD-009 惰性化——resolve/reopen 失败时刷新一次，热路径零 glob\n- 新设备: fd=-1, readEvent/getStats 懒打开\n- 离线设备: close(fd) + 移除\n- 在线且路径不变: 复用 entry (含 fd)\n- R-11: getStats 复用 readEvent 持久 fd（懒打开缓存）
@enduml
```
...
```cpp
// hal_service.h:30 — struct DeviceEntry
struct DeviceEntry {
...
note over B: 下一个调用在前一个返回后才进入\nLCD-010：串行化是设计意图（fd 表/read_event\n未做并发审计），扩容前须加互斥
...
if (ret < 0 && errno == EINTR?) then (是)
    :EINTR 瞬时噪声，重试;<<#lightgreen>>
endif
repeat while (ret < 0 && errno == EINTR?) is (是)
->否;
if (ret < 0?) then (是)
    :返回 -1 (poll 错误);
    stop
...
**排空策略说明：** 内核环形缓冲区大小为 32 条事件（`VENDOR_LECHAO_USBD_EVENT_BUF_SIZE`）。用户态消费不及时时可能积压多条。`read_event` 循环 read 直到排空，只保留最后一条（最新），中间事件丢弃并打印警告日志，**丢弃数量经 `dropped` 出参透出**（[`device_io.cpp` 的 `read_event(fd, &ev, timeout, &dropped)`](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lciod/hal/device_io.cpp)，`hal_service.cpp:309-313` 经 dropped 记录日志使事件完整性可见）——排空丢弃不再静默，积压可观测。LCD-016：poll 被 EINTR 打断时 `do-while` 重试而非返回错误（信号打断是瞬时噪声，报错会让上层把噪声当设备故障），见 [`device_io.cpp:161-167`](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lciod/hal/device_io.cpp#L161)。
...
│   ├── IoStats.aidl                       # 统计数据 parcelable（25字段）
...
| 文件 | 职责 | 关键函数/类 | 行数 |
|------|------|------------|------|
| `hal_service.cpp` | AIDL 实现、设备缓存、字段映射、进程入口 | `IoHalImpl` / `main()` | 308 |
| `device_io.cpp` | 底层 IO 封装（glob 枚举、带重试 open、ioctl、排空 read、超时钳位） | `list_devices()` / `read_event()` / `clamp_read_timeout_ms()` | 202 |
| `device_io.h` | IO 操作 C 风格 API 声明 | — | 102 |
...
| `vendor_lechao_usbd-ioctl.h` | 内核共享 ABI（struct/event/ioctl 命令） | `struct vendor_lechao_usbd_stats` | 183 |
| `IIoHal.aidl` | VINTF 稳定 AIDL 接口 | `listDevices` / `getStats` / `readEvent` 等 | 47 |
| `IoStats.aidl` | 统计 parcelable（25 字段） | — | 51 |
...
**设计思路：** `mDeviceMap`（minor → DeviceEntry）缓存所有在线设备信息。LCD-009 惰性化后不再"每次 AIDL 调用前同步"：`refresh_devices()` 仅在 resolve 失败（新设备插入）或 reopen 失败（设备移除）时调用一次，热路径（daemon monitor 每 50ms 调 `readEvent`，20Hz/设备）零 `glob(3)` 扫描。刷新语义本身不变：在线且路径不变的设备复用已有 entry（含持久 fd），新设备创建 entry（fd=-1 待懒打开），离线设备关闭 fd 并移除。
...
```cpp
// hal_service.cpp:273 — readEvent 中懒打开持久 fd
if (entry->fd < 0) {
    entry->fd = ::open_device(entry->path.c_str());
}

// hal_service.cpp:291 — 设备移除时关闭 fd 并标记 -1
...
```cpp
// device_io.cpp:182 — poll + 循环 read 排空
...
> **ABI v2 字段映射说明：** 内核 `struct vendor_lechao_usbd_stats`（ABI v2）含 27 个业务字段 + `reserved[3]`。`event_drop_count`（环形缓冲区溢出丢弃计数）已映射到 AIDL `IoStats.eventDropCount` 透出（LCD-012，IoStats 25 字段）；`peak_rate`、`last_update` 仍不映射——HAL 仅选取上层监控需要的字段，内核保留的诊断字段可通过 usb-verify 工具直接 ioctl 读取。
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| **v1.4** | **2026-09-23** | **getStats 持久 fd 复用（R-11 方向 2）+ 排空丢弃透出**：(1) **getStats 复用持久 fd**：`IoHalImpl::getStats` 复用 `DeviceEntry.fd`（懒打开并缓存，ioctl 与 read/poll 共用同一 fd 不冲突），设备移除（ENODEV/EIO）时关闭 fd 置 -1——消除逐调用 open/close（[`hal_service.cpp:143-162`](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lciod/hal/hal_service.cpp#L143)）；`DeviceEntry` 注释同步更新；(2) **排空丢弃透出**：`read_event(fd, &ev, timeout, &dropped)` 出参透出排空丢弃条数，`readEvent` 经 dropped 记录日志（`hal_service.cpp:309-313`）——丢弃不再静默，积压可观测；(3) **eventDropCount 持久化**：`getStats` 返回 `raw.event_drop_count` 透出（LCD-012 字段，[`hal_service.cpp:195`](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lciod/hal/hal_service.cpp#L195)） |
