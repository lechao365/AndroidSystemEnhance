...
| 模块 | 职责 | 关键抽象 |
|------|------|---------|
| `hal_service.cpp` | AIDL 接口实现、设备缓存管理、字段映射 | `class IoHalImpl` — [hal_service.cpp:66](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lciod/hal/hal_service.cpp#L66) |
| `device_io.cpp` | 底层 IO 封装（glob/ioctl/poll/read） | `list_devices()` / `read_event()` — [device_io.cpp:29](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lciod/hal/device_io.cpp#L29) |
...
```cpp
// main_lciod_hal.cpp:26 — 单线程 Binder 线程池
...
// hal_service.cpp:278 — 设备移除时关闭 fd 并标记 -1
...
| Binder 线程池 | 1 | `main_lciod_hal.cpp:26` | 单线程串行执行 |
