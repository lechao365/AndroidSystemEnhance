...
- 模块源码：[`code/rpi5/others/usb-verify/`](../../code/rpi5/others/usb-verify/)
...
| 层 | 模块 | 职责 | 关键函数 |
|----|------|------|---------|
| cli | `main.c` | 命令路由器，根据 `fv_command_kind` 分发到对应子命令 | `main()` — [main.c:38](../../code/rpi5/others/usb-verify/src/cli/main.c) |
| cli | `parse.c` | 两阶段参数解析器：子命令识别 + 选项参数遍历 | `fv_parse_args()` / `usage()` / `parse_event_type()` — [parse.c:112](../../code/rpi5/others/usb-verify/src/cli/parse.c) |
| cli | `output.c` | 文本表格与 JSON 双格式输出 | `output_stats()` / `output_check_report()` / `output_degrade_check()` — [output.c:38](../../code/rpi5/others/usb-verify/src/cli/output.c) |
| device | `usbd_device.c` | 系统调用封装（open/close/ioctl/poll/read），返回 `-errno` | `usbd_open()` / `usbd_get_stats()` / `usbd_read_event()` — [usbd_device.c:23](../../code/rpi5/others/usb-verify/src/device/usbd_device.c) |
| check | `stats_check.c` | 统计阈值断言引擎 | `fv_check_stats()` / `add_entry()` — [stats_check.c:40](../../code/rpi5/others/usb-verify/src/check/stats_check.c) |
| check | `event_check.c` | 事件轮询等待 + 事件类型匹配 + 降级检查 | `fv_wait_for_event()` / `fv_check_event()` / `fv_check_degrade()`* — [event_check.c:29](../../code/rpi5/others/usb-verify/src/check/event_check.c) |

> *`fv_check_degrade()` 当前为死代码：`CHECK_DEGRADE` 分支（main.c:167-177）实际调用的是 `output_degrade_check()`（output.c:194-235），两者逻辑等价。`fv_check_degrade()` 保留供未来单元测试或程序化断言使用。
| common | `types.h` | 断言报告数据结构（`fv_check_entry` / `fv_check_report`） | — [types.h:27](../../code/rpi5/others/usb-verify/src/common/types.h) |
| common | `error.h` | 6 级退出码枚举（`FV_OK` 到 `FV_ERR_CHECK`） | — [error.h:22](../../code/rpi5/others/usb-verify/src/common/error.h) |
| common | `util.c` | 时间转换辅助函数 | `fv_timespec_to_ms()` — [util.c:4](../../code/rpi5/others/usb-verify/src/common/util.c) |
| abi | `vendor_lechao_usbd-ioctl.h` | 内核-用户态共享 ABI（stats/config/event 结构体 + ioctl 命令） | — [vendor_lechao_usbd-ioctl.h:45](../../code/rpi5/others/usb-verify/include/vendor_lechao_usbd-ioctl.h) |
| abi | `fv_ioctl_compat.h` | 用户态类型桥接层（`u8/u16/u32/u64` typedef） | — [fv_ioctl_compat.h:27](../../code/rpi5/others/usb-verify/include/fv_ioctl_compat.h) |
...
| `event_check.c` | 事件等待 + 降级检查 | `fv_wait_for_event()` / `fv_check_event()` / `fv_check_degrade()`（当前死代码，见下方说明）/ `add_degrade_entry()` | 133 |
...
| `util.h` | 辅助函数声明 | — | 9 |
| `vendor_lechao_usbd-ioctl.h` | 内核-用户态共享 ABI | `struct vendor_lechao_usbd_stats` / `struct vendor_lechao_usbd_config` / `struct vendor_lechao_usbd_event` / 4 个 IOC 宏 | 184 |
...
| 输出 | `fault-verify` | 单一二进制（**gitignore 不入库**——`.gitignore` 含 `fault-verify`，构建产物由 `make clean` 清除，见下） |
...
> **构建产物不入库（R-13 配套）：** 仓库根 `.gitignore` 含 `fault-verify`（构建产物，make 生成；见 `Makefile clean` 目标）。此前该二进制曾误入库，本批删除并加 gitignore，源码编译产物不落入版本控制。
...
**字节序编译守卫（R-13 配套）：** 头文件补非小端 `#error` 编译守卫（[`vendor_lechao_usbd-ioctl.h:152`](../../code/rpi5/others/usb-verify/include/vendor_lechao_usbd-ioctl.h#L152)），与内核真相源 `lciod_usbd-ioctl.h` 同款——事件/统计结构按小端契约裸传输（LCD-004），大端环境直接编译失败防隐性字节序错误；三方（内核/HAL/usb-verify）共享同一守卫，双侧同步禁止单侧改布局。
...
> **输出端限制：** `parse_event_type()` 支持解析 `"degrade" → 6`，但 `output.c:21-32` 的 `event_type_name()` switch 未包含 `case 6`，落入 `default: return "unknown"`。因此 `event wait --type degrade` 匹配到事件时，JSON 输出的 `event_type` 字段会显示 `"unknown"` 而非 `"rate_degraded"`。使用 `check degrade` 子命令可规避此限制。

> **stats get 字段覆盖：** `output_stats()`（output.c:38-90）输出 17 个字段，不输出 `read_ns`、`write_ns`、`last_update`、`last_event_type`、`event_drop_count`（ABI v2 新增，环形缓冲区溢出丢弃计数）。如需查看完整 stats，可使用 `ioctl` 直接读取或扩展 `output_stats()`。
...
```bash
cd code/rpi5/others/usb-verify/
make
```
