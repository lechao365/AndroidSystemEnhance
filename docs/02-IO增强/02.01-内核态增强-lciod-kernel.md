...
| 模块 | 职责 | 关键抽象 |
|------|------|---------|
| `lciod_usbd.c` | 字符设备注册、设备生命周期管理、vendor notifier 注册 | `vendor_lechao_usbd_vendor_notifier` — [lciod_usbd.c:683](../../code/rpi5/kernel/new/vendor/lechao/LcIod/lciod_usbd.c#L683) |
| `lciod_usbd-stats.c` | notifier 回调、统计更新、degrade 检测、事件推送、LcView 打点 | `vendor_lechao_usbd_handle_event` — [lciod_usbd-stats.c:508](../../code/rpi5/kernel/new/vendor/lechao/LcIod/lciod_usbd-stats.c#L508) |
...
**两种 degrade 判定条件**（源码 [lciod_usbd-stats.c:627-645](../../code/rpi5/kernel/new/vendor/lechao/LcIod/lciod_usbd-stats.c#L627)）：

```c
// lciod_usbd-stats.c:627-645
...
| 不变量 | 保证机制 | 代码位置 |
|--------|---------|---------|
| 设备不重复注册 | 双重检查（mutex 外 + mutex 内） | [`lciod_usbd.c:708-725`](../../code/rpi5/kernel/new/vendor/lechao/LcIod/lciod_usbd.c#L708) |
| kref 归零时才释放 | `kref_put` + `device_release` 回调 | [`lciod_usbd.c:491-497`](../../code/rpi5/kernel/new/vendor/lechao/LcIod/lciod_usbd.c#L491) |
| removing 标志原子访问 | `READ_ONCE`/`WRITE_ONCE` | [`lciod_usbd.c:180`](../../code/rpi5/kernel/new/vendor/lechao/LcIod/lciod_usbd.c#L180) |
...
| 环满时丢弃最旧 | head == tail 时 tail 推进 | [`lciod_usbd-stats.c:405-409`](../../code/rpi5/kernel/new/vendor/lechao/LcIod/lciod_usbd-stats.c#L405) |
| `lcview_trace_*` 字段完整性 | 任一 add 失败整体丢弃事件（KRN-016），防用户态 schema 解析错位 | [`lciod_usbd.c:86-89`](../../code/rpi5/kernel/new/vendor/lechao/LcIod/lciod_usbd.c#L86) |
...
| 文件 | 职责 | 关键函数 | 行数 |
|------|------|---------|------|
| `lciod_usbd.c` | 字符设备注册、设备生命周期、vendor notifier | `vendor_lechao_usbd_vendor_notifier`、`vendor_lechao_usbd_monitor_init` | 988 |
| `lciod_usbd-stats.c` | notifier 回调、统计更新、degrade 检测 | `vendor_lechao_usbd_handle_event`、`vendor_lechao_usbd_update_degrade_context_locked` | 731 |
| `lciod_read_logic.c` | read 排空判定纯函数（内核 API 剥离） | `lciod_nonblock_read_decision` | 15 |
| `lciod_read_logic.h` | 纯函数声明（内核与 host 单测共用） | — | 31 |
...
| `tests/lciod_read_logic_host_test.c` | read 排空判定 host 单测（四象限语义） | `main` | 59 |
...
| 配置项 | 文件 | 说明 |
|--------|------|------|
| 设备节点权限 | [`lciod_usbd.c:121`](../../code/rpi5/kernel/new/vendor/lechao/LcIod/lciod_usbd.c#L121) | `devnode` 设置 0666 |
| ueventd | `ueventd.rpi5.rc` | 可添加 `/dev/vendor_lechao_usbd* 0666` |
...
// lciod_usbd.c:545-549 (错误路径：kref 初始化前需手动 ida_free)
if (!us->pusb_dev) {
...
- **KRN-004 shutdown 不越过非空判定**：断连后 ring 中可能仍有未消费的尾部事件，read() 先 drain 这些事件（返回事件）、ring 清空后才返回 0 (EOF)——判定逻辑抽至 `lciod_read_logic.c`（`lciod_nonblock_read_decision` 纯函数，host 单测覆盖四象限语义：返回 1→`-EAGAIN` 空环重试、0→0 EOF、-1→非空 drain，[`lciod_usbd.c:249-251`](../../code/rpi5/kernel/new/vendor/lechao/LcIod/lciod_usbd.c#L249)）
- **KRN-009 copy_to_user 失败回滚 event_tail**：read 取出事件后 `copy_to_user` 失败时回滚 `event_tail`，事件留在环中供下次重试而非丢失（[`lciod_usbd.c:281-294`](../../code/rpi5/kernel/new/vendor/lechao/LcIod/lciod_usbd.c#L281)；LCD-008 单读者下回滚不会导致重复投递给其他读者）
- **LCD-008 单读者消费语义**：`event_tail` 是 per-device 共享状态，多读者并发 open 时构成共享消费队列（每条事件被随机分配给其中一个读者，不复制不回放）。当前实际单读者由 sepolicy 保证（`lechao_lciod_hal_device` 的 chr_file 访问仅授予 HAL domain，[`lciod_usbd.c:167-176`](../../code/rpi5/kernel/new/vendor/lechao/LcIod/lciod_usbd.c#L167)）；若未来出现多读者需求，须改为 per-fd 消费位或事件复制，不能依赖现有语义
...
|-------------|------|------|-----------|---------|
| `usb_stor_vendor_nh` | blocking | PROBE/DISCONNECT | 进程上下文 | [`lciod_usbd.c:914`](../../code/rpi5/kernel/new/vendor/lechao/LcIod/lciod_usbd.c#L914) |
| `us->notifier` | atomic | START/END/ERROR/STALL/TIMEOUT/CORRUPT/RESET | 任意上下文 | [`lciod_usbd.c:596`](../../code/rpi5/kernel/new/vendor/lechao/LcIod/lciod_usbd.c#L596) |
...
实现见 [`lciod_usbd.c:76-115`](../../code/rpi5/kernel/new/vendor/lechao/LcIod/lciod_usbd.c#L76) 和 [`lciod_usbd-stats.c:191-352`](../../code/rpi5/kernel/new/vendor/lechao/LcIod/lciod_usbd-stats.c#L191)。

**KRN-016（add 失败聚合处理）：** `lcview_trace_probe` / `lcview_trace_disconnect` 对字段 `add_*` 采用聚合返回码（`rc |= add_int(...)` / `rc |= add_str(...)`）——任一字段 add 失败都会使用户态按 schema 解析错位，故只要 `rc || commit` 任一失败即整体 `lcview_builder_cancel` 丢弃事件，不发射残缺记录（`lciod_usbd.c:72-111`）。
...
> `file_operations` 定义：[`lciod_usbd.c:472`](../../code/rpi5/kernel/new/vendor/lechao/LcIod/lciod_usbd.c#L472)
