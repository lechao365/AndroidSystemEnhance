...
```c
// service.cpp:54-63（计算字段，收敛为纯函数 ComputeAverageRate 供单测）
...
**固定节拍（LCD-018）：** 旧实现 `sleep_for(50ms)` 为相对睡眠，处理耗时（遍历设备 × readEvent 全 timeout 时周期可 >250ms）逐轮累积，10s 统计节拍实际 >50s 漂移。改为 `steady_clock` 固定节拍：`next_tick += kTickPeriod` 后 `sleep_until(next_tick)`，处理耗时从睡眠中扣除；单轮超时则立即追平，不放大漂移（[`service.cpp:257-275`](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lciod/daemon/service.cpp#L257)）。

**HAL 断连节流（LCD-007）：** HAL 故障时每 tick 打 `not connected` 日志会淹没 logcat（50 条/秒），改为 200 tick（10s）一条节流告警，与 hal_client 重连失败节流同策略（[`service.cpp:279-283`](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lciod/daemon/service.cpp#L279)）。**LCD-020：** `listDevices` 返回的路径解析失败时降级为 `LC_ALOGD` debug 日志（非每轮 WARN 刷屏，[`service.cpp:140`](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lciod/daemon/service.cpp#L140)）。

**监控参数：**

| 参数 | 值 | 定义位置 |
|------|-----|---------|
| tick 间隔 | 50ms | `service.cpp:257`（`kTickPeriod`，LCD-018 固定节拍） |
| 统计刷新周期 | 200 tick（10s） | `service.cpp:282` |
| readEvent timeout | 50ms | `service.cpp:306` |
...
| detach 线程随进程终止 | 进程非 oneshot 由 init 自动重启（LCD-005），异常退出时线程随进程自动终止 | [`service.cpp:253`](../../code/rpi5/aosp/new/vendor/lechao/services/lechao_lciod/daemon/service.cpp#L253) |
...
// service.cpp:76-99（字段投影，收敛为纯函数 ProjectSystemIoStats）
