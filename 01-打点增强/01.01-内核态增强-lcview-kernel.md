...
> 驱逐实现：[`ring_evict_one`](./../patchs/rpi5/kernel/new/vendor/lechao/LcView/lcview_ring.c#L197)，批量逻辑：[`lcview_ring.c:293`](./../patchs/rpi5/kernel/new/vendor/lechao/LcView/lcview_ring.c#L293)
...
package "用户空间" as userspace {
    object "lechao_lcview_hal\n(vendor域, epoll读取)" as HAL
    object "lechao_lcview\n(system域, Binder拉取)" as DAEMON
}

LCVIEW --> CHRDEV : **创建设备节点**
CHRDEV --> HAL : **open/read/poll/ioctl**
HAL --> DAEMON : **Binder AIDL getBatch()**
@enduml
```
...
`event_id` 是用户态 schema 查找键，当前已定义 13 个事件 ID（[`lcview_events.h:47-69`](./../patchs/rpi5/kernel/new/vendor/lechao/LcView/lcview_events.h#L47)），覆盖 USB（1,4-13）、GPIO（2）、SENSOR（3）三类来源。传输中不含任何字段名字符串，一条含 4 个字段的 USB 连接事件序列化后仅 ~50 字节，而传统 `printk("device=%s port=%d vid=0x%04x pid=0x%04x")` 需 ~80 字节。
...
`add_*` 系列直接写 `b->buf[data_offset]`，每次写入后推进 `data_offset` 并递增 `field_count`。固定长度字段（INT32/INT64/FLOAT）共用 [`builder_write_field()`](./../patchs/rpi5/kernel/new/vendor/lechao/LcView/lcview_builder.c#L134)（`lcview_builder.c:134`），变长字段（STRING/BINARY）有独立函数处理 2 字节长度前缀。
...
`min_level` 默认 `LCVIEW_LEVEL_DEBUG(0)`（不过滤），由用户态通过 `LCVIEW_SET_LEVEL` ioctl 动态调整（[`lcview_main.c:216`](./../patchs/rpi5/kernel/new/vendor/lechao/LcView/lcview_main.c#L216)）。对比传统 `printk` 的"先格式化再判断 level"，此设计在 DEBUG 级别被过滤时节省了完整的 `kmalloc(4100B) + 字段序列化 + spinlock 写入` 开销。
...
读取路径的关键步骤（[`lcview_ring_read`](./../patchs/rpi5/kernel/new/vendor/lechao/LcView/lcview_ring.c#L350)，`lcview_ring.c:350`）：
...
[`ring_evict_one`](./../patchs/rpi5/kernel/new/vendor/lechao/LcView/lcview_ring.c#L197)（`lcview_ring.c:197`）读取被驱逐记录的 4B 长度前缀，推进 `read_pos`，递增 `overrun_cnt`。包含防御性处理：若长度前缀异常（0 或 > ring->size），使用保守默认值（前缀 + 记录头 = 20B）跳过，避免数据损坏导致永久性错乱。
                           ...
                           const uint8_t *src, uint32_t len)
{
    if (pos + len <= ring->size) {
        memcpy(ring->buf + pos, src, len);              // 无回绕：一次拷贝
    } else {
        uint32_t part1 = ring->size - pos;              // 回绕：分两段
        memcpy(ring->buf + pos, src, part1);            //   尾部 → 末尾
        memcpy(ring->buf, src + part1, len - part1);    //   头部 ← 开头
    }
}
```

[`ring_memcpy_out`](./../patchs/rpi5/kernel/new/vendor/lechao/LcView/lcview_ring.c#L79)（`lcview_ring.c:79`）为对称的读取版本。写入时先 `ring_memcpy_in` 写 4B 长度前缀，再 `ring_memcpy_in` 写记录数据（`lcview_ring.c:310-316`），两次调用各自独立处理回绕。`write_pos` 推进使用 `% ring->size` 取模。
...
| 不变量 | 守护机制 | 源码位置 |
|--------|---------|---------|
| 单实例 open | `atomic_cmpxchg(&device_opened, 0, 1)` — 无锁 CAS，多线程并发仅一个成功 | [`lcview_main.c:99`](./../patchs/rpi5/kernel/new/vendor/lechao/LcView/lcview_main.c#L99) |
| commit 幂等 | `committed` 标志，二次 commit 返回 `-EINVAL` | [`lcview_builder.c:277`](./../patchs/rpi5/kernel/new/vendor/lechao/LcView/lcview_builder.c#L277) |
| 空环判定 | `ring_avail_write()` 返回 `ring->size - used - 1`，预留 1 字节 | [`lcview_ring.c:66`](./../patchs/rpi5/kernel/new/vendor/lechao/LcView/lcview_ring.c#L66) |
| 单条 ≤ 4KB | `LCVIEW_BUILDER_MAX_SIZE` 边界检查，溢出返回 `-ENOSPC` | [`lcview_internal.h:35`](./../patchs/rpi5/kernel/new/vendor/lechao/LcView/lcview_internal.h#L35) |
| shutdown 顺序 | 先设 `shutdown=true` 再 `wake_up`，确保等待中的 reader 退出后才释放内存 | [`lcview_ring.c:172`](./../patchs/rpi5/kernel/new/vendor/lechao/LcView/lcview_ring.c#L172) |
| 字符串 NULL 安全 | `compute_str_len(NULL)` 返回 0，NULL 字符串写为空串 | [`lcview_builder.c:57`](./../patchs/rpi5/kernel/new/vendor/lechao/LcView/lcview_builder.c#L57) |
...
8 个 Builder API 通过 `EXPORT_SYMBOL` 导出到内核符号表（[`lcview_main.c:280-298`](./../patchs/rpi5/kernel/new/vendor/lechao/LcView/lcview_main.c#L280)），其他模块通过 `extern` 声明或包含 `lcview_events.h` 调用：
...
构建集成通过 Kconfig/Makefile 独立配置（`Kconfig` 中 `CONFIG_LCVIEW=y`），`module_init(lcview_init)` 映射为 `device_initcall`（[`lcview_main.c:366`](./../patchs/rpi5/kernel/new/vendor/lechao/LcView/lcview_main.c#L366)），随 vmlinux 启动。上层 Kconfig 仅需 `source "vendor/lechao/LcView/Kconfig"` 一行即可接入。
...
| **v3.7** | **2026-06-27** | **测试防护体系建立**：(1) 提炼 4 类 bug 模式为硬规范 [CXX-001~004](../engineering/harness/rules/cxx-coding-rules.md)；(2) 用户态 daemon/HAL 单元测试（gmock 框架）落地 `tests/` 目录；(3) `mk_rpi5_full_image.sh` 编译阶段集成测试模块编译校验 |
